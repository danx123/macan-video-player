...
class ModernVideoPlayer(QWidget):
    ...
    def _update_control_states(self):
        ...
        # PERBAIKAN: Tombol kontrol aktif selama ada media, meskipun statusnya 'Stopped'
        is_media_loaded = self.player.mediaStatus() != QMediaPlayer.MediaStatus.NoMedia
        
        # --- Splash Screen Logic ---
        # Tampilkan splash hanya jika tidak ada media atau jika media dihentikan (dan kita tidak ingin bisa memutarnya lagi)
        # Dengan logika baru, splash hanya muncul saat tidak ada media sama sekali.
...
if __name__ == "__main__":
    ...
    # =================== FIX STARTS HERE ===================
    # Check if a command-line argument (file path) was provided
    if len(sys.argv) > 1:
        file_path_to_open = sys.argv[1]
        # Use QTimer.singleShot to ensure the main window is fully initialized
        # before attempting to load the media. This prevents startup issues.
