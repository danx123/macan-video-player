...
class YouTubeDLWorker(QObject):
    """
    Worker untuk menjalankan yt-dlp di thread terpisah agar tidak membekukan UI.
    """
...
class PlaylistWidget(QWidget):
    """
    Jendela terpisah untuk mengelola playlist video.
    """
    ...
    def __init__(self, parent=None):
        ...
        # Aktifkan fitur drag-and-drop di widget playlist
    ...
    # --- Implementasi Drag-and-Drop untuk Playlist ---
...
class ModernVideoPlayer(QWidget):
    """
    Jendela utama video player dengan semua kontrol.
    """
    def __init__(self):
        super().__init__()
        ...
        # Baca konfigurasi sebelum membuat UI
        self._load_config()
        self._setup_ui()
        self._connect_signals()
        ...
        # Aktifkan fitur drag-and-drop di widget utama dan video
        self.setAcceptDrops(True)
    ...
    def _load_config(self):
        """Memuat konfigurasi volume dan playlist dari file JSON."""
        try:
    ...
    def _setup_ui(self):
        ...
        # Set nilai awal dari config yang sudah dimuat sebelumnya
        ...
        # Penataan Layout Baru
        self.controls_container = QWidget()
        
        # Layout atas untuk buka file/url
        top_controls_layout = QHBoxLayout()
        top_controls_layout.addWidget(self.btn_open)
        top_controls_layout.addWidget(self.btn_open_srt)
        top_controls_layout.addStretch(1)
        top_controls_layout.addWidget(self.url_input, 4)
        top_controls_layout.addWidget(self.btn_load_url)
        top_controls_layout.addWidget(self.btn_show_playlist)
        
        # Layout slider dan waktu
        slider_time_layout = QHBoxLayout()
        slider_time_layout.addWidget(self.position_slider)
        slider_time_layout.addWidget(self.time_label)

        # Layout bawah untuk semua kontrol
        combined_controls_layout = QHBoxLayout()
        combined_controls_layout.addStretch()
        combined_controls_layout.addWidget(self.btn_prev_playlist)
        combined_controls_layout.addWidget(self.btn_skip_back)
        combined_controls_layout.addWidget(self.btn_play_pause)
        combined_controls_layout.addWidget(self.btn_skip_forward)
        combined_controls_layout.addWidget(self.btn_next_playlist)
        combined_controls_layout.addWidget(self.btn_stop)
        
        combined_controls_layout.addSpacing(20)
        
        combined_controls_layout.addWidget(self.btn_speed)
        combined_controls_layout.addWidget(self.btn_audio_mode)
        combined_controls_layout.addWidget(self.btn_mute)
        combined_controls_layout.addWidget(self.volume_slider, 2)
        combined_controls_layout.addWidget(self.btn_fullscreen)
        combined_controls_layout.addStretch()
        
        container_layout = QVBoxLayout()
        container_layout.addWidget(self.title_label)
        container_layout.addLayout(top_controls_layout)
        container_layout.addLayout(slider_time_layout)
        container_layout.addLayout(combined_controls_layout)
    ...
    def dragEnterEvent(self, event):
        """Menerima event drag jika data berisi URL (path file)."""
        if event.mimeData().hasUrls():
            event.acceptProposedAction()
        else:
            event.ignore()
    ...
    def dropEvent(self, event):
        """Memuat file video yang di-drop."""
        for url in event.mimeData().urls():
            file_path = url.toLocalFile()
            if os.path.exists(file_path):
                # Validasi sederhana apakah file tersebut adalah video
                file_extension = os.path.splitext(file_path)[1].lower()
                if file_extension in ['.mp4', '.mkv', '.webm', '.avi']:
    ...
    def _update_playlist_nav_buttons(self):
        """Mengaktifkan/menonaktifkan tombol next/prev playlist."""
        playlist_data = self.playlist_widget.get_playlist_data()
        current_index = self.playlist_widget.get_current_index()
    ...
    def resizeEvent(self, event):
        ...
        # Menyesuaikan posisi label subtitle saat jendela diubah ukurannya
    ...
    def _set_volume(self, value):
        ...
        self._save_config() # Simpan volume setiap kali berubah
    ...
    def _handle_media_status_changed(self, status):
        ...
        if status == QMediaPlayer.MediaStatus.LoadedMedia and not self.is_fullscreen:
            ...
            # Mendapatkan ukuran layar yang tersedia (termasuk memperhitungkan taskbar)
            available_geom = self.screen().availableGeometry()
            screen_width = available_geom.width()
            screen_height = available_geom.height()
            
            # Mendapatkan tinggi dari panel kontrol
            controls_height = self.controls_container.sizeHint().height()
            
            # Menghitung ruang yang tersedia untuk video
            max_video_height = screen_height - controls_height
            max_video_width = screen_width
            
            # Menentukan ukuran video yang disesuaikan
            scaled_video_size = video_res.scaled(max_video_width, max_video_height, Qt.AspectRatioMode.KeepAspectRatio)
            
            # Menghitung ukuran jendela baru
            final_width = scaled_video_size.width()
            final_height = scaled_video_size.height() + controls_height
            
            # Mencegah jendela menjadi terlalu kecil (opsional)
            if final_width < 400: final_width = 400
            if final_height < 300: final_height = 300
    ...
    def closeEvent(self, event):
        """Simpan konfigurasi saat aplikasi ditutup."""
        self._save_config()
        event.accept()
