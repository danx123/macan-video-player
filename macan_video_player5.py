...
class YouTubeDLWorker(QObject):
    finished = pyqtSignal(str, str) # Mengirimkan url video dan error (jika ada)
    ...
    def run(self):
        if not YoutubeDL:
            self.finished.emit(None, "Pustaka yt-dlp tidak terinstal.")
            return

        try:
            # <<< FIX DI SINI >>>
            # Logika ini diubah untuk secara andal menemukan stream video TERBAIK
            # yang memiliki BAIK video MAUPUN audio. Pustaka yt-dlp dengan opsi
            # 'bestvideo+bestaudio/best' seringkali menghasilkan URL video-only
            # di tingkat atas metadata jika kualitasnya paling tinggi.
            # QMediaPlayer tidak dapat menggabungkan audio dan video terpisah,
            # jadi kita harus secara eksplisit mencari format yang sudah digabung.
            ydl_opts = {
            ...
            with YoutubeDL(ydl_opts) as ydl:
                ...
                # Kita akan mencari melalui daftar format yang tersedia, dari kualitas
                # tertinggi ke terendah, dan memilih yang pertama yang memiliki video dan audio.
                formats = info_dict.get('formats', [info_dict])
                
                for f in reversed(formats):
                    # Kriteria: harus punya URL, codec video, dan codec audio
                    if f.get('url') and f.get('vcodec') != 'none' and f.get('acodec') != 'none':
                        self.finished.emit(f['url'], None)
                        return # Berhasil, keluar dari fungsi

                # Jika loop selesai tanpa menemukan format yang cocok
                self.finished.emit(None, "Tidak dapat menemukan URL streaming yang valid dengan video dan audio.")

        except Exception as e:
            # Menangkap kesalahan dari yt-dlp (misalnya, URL tidak valid, video pribadi)
            self.finished.emit(None, f"Error dari yt-dlp: {str(e)}")
...
class ModernVideoPlayer(QWidget):
    def __init__(self):
        ...
        # Timer untuk menyembunyikan kursor
    ...
    def _setup_ui(self):
        ...
        self.btn_play_pause.setToolTip("Putar / Jeda (Spasi)")
    ...
    def _load_from_url(self):
        ...
        self.url_input.setPlaceholderText("Sedang mengambil info video...")
    ...
    def _on_youtube_dl_finished(self, video_url, error):
    ...
    def keyPressEvent(self, event):
        ...
        elif event.key() == Qt.Key.Key_Space and self.btn_play_pause.isEnabled(): self._toggle_play_pause()
