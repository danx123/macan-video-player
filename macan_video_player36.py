...
class ThumbnailGenerator(QObject):
    ...
    def generate(self, video_path, timestamp_ms, request_time):
        ...
        # --- FIX: Optimasi command FFmpeg untuk generate lebih cepat ---
        # 1. '-ss' ditempatkan sebelum '-i' untuk "input seeking" yang jauh lebih cepat.
        # 2. Codec diubah dari 'png' ke 'mjpeg' yang lebih cepat untuk encoding frame tunggal.
        command = [
            'ffmpeg',
            '-ss', str(timestamp_sec), # Pindah ke sebelum -i untuk fast seeking
            '-i', video_path,
            '-vframes', '1',          # Hanya 1 frame
            '-s', '160x90',            # Ukuran output
            '-f', 'image2pipe',        # Output ke pipe
            '-vcodec', 'mjpeg',        # Gunakan mjpeg untuk kecepatan
            '-'                        # Output ke stdout
        ]
        ...
        try:
            ...
            pixmap = QPixmap()
            # --- FIX: Gunakan format 'JPG' karena codec diubah ke mjpeg ---
            pixmap.loadFromData(process.stdout, "JPG")
...
class ModernVideoPlayer(QWidget):
    ...
    def __init__(self):
        ...
        # --- FIX: Aktifkan mouse tracking di seluruh jendela utama untuk fullscreen ---
        self.setMouseTracking(True)
    ...
    def _hide_controls(self):
        # Sembunyikan hanya jika sedang fullscreen dan video sedang diputar
        if self.is_fullscreen and self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState:
    ...
    def _update_control_states(self):
        ...
        # Untuk fullscreen, visibilitas kontrol diatur oleh mouseMoveEvent
    ...
    def _toggle_fullscreen(self):
        ...
        if self.is_fullscreen:
            self.showFullScreen()
            # --- FIX: Sembunyikan kontrol saat masuk fullscreen ---
            self.controls_container.setVisible(False) 
    ...
    # --- FIX: Logika baru untuk menangani kursor dan kontrol di mode fullscreen ---
    def mouseMoveEvent(self, event):
        """Menangani pergerakan mouse untuk menampilkan kursor dan kontrol."""
        if self.is_fullscreen:
            self._show_cursor()  # Selalu tampilkan kursor saat mouse bergerak
            self.cursor_hide_timer.start()  # Mulai ulang timer untuk menyembunyikan kursor

            # Tentukan "hot zone" di bagian bawah layar untuk menampilkan kontrol
            mouse_y = event.pos().y()
            # Ambil tinggi bilah kontrol, beri sedikit buffer
            control_bar_height = self.controls_container.height() + 10
            screen_height = self.height()

            # Jika kursor berada di area bawah, tampilkan kontrol
            if mouse_y >= (screen_height - control_bar_height):
                if not self.controls_container.isVisible():
                    self.controls_container.setVisible(True)
                # Hentikan timer hide-controls agar tetap terlihat selama kursor di bawah
                self.controls_hide_timer.stop()
            else:
                # Jika kursor di luar area bawah, mulai timer untuk sembunyikan kontrol
    ...
    def eventFilter(self, source, event):
        ...
        if source is self.video_widget:
            # Logika untuk menampilkan/menyembunyikan kontrol saat tidak fullscreen
            if not self.is_fullscreen:
                if event.type() == QEvent.Type.Enter:
                    ...
                    if self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState:
                         # Di mode normal, kita tidak menyembunyikan kontrol
                        pass
                    return True
