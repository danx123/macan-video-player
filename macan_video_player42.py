...
from PyQt6.QtWidgets import (
    ...
    QAbstractItemView, QDialog, QStackedWidget, QGraphicsDropShadowEffect
)
...
class MiniPlayerWindow(QWidget):
    ...
    def _sync_volume_slider(self):
        ...
        if not self.volume_slider.isSliderDown():
            if is_muted: self.volume_slider.setValue(0)
            else: self.volume_slider.setValue(int(volume_float * 100))
        self._update_volume_icon()
...
class ModernVideoPlayer(QWidget):
    ...
    def __init__(self):
        ...
        self.history = []
        self.resume_points = {}
        self.pending_resume_position = -1
    ...
    def _setup_resume_feature(self):
        self.resume_save_timer = QTimer(self)
        self.resume_save_timer.setInterval(5000)
        self.resume_save_timer.timeout.connect(self._save_current_position)
    ...
    def _setup_player(self):
        ...
        self.video_sink.videoFrameChanged.connect(self.process_frame_for_mini_player)
    ...
    def _load_config(self):
        try:
            ...
            if saved_theme in self.theme_names: self.current_theme_index = self.theme_names.index(saved_theme)
            self.history = config.get('history', [])
            self.history_window.history_data = self.history
            self.history_window.populate_list()
            self.resume_points = config.get('resume_points', {})
    ...
    def _setup_ui(self):
        ...
        else: self.splash_label.setText("Macan Player")
        self.splash_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.splash_label.setStyleSheet("background-color: black; font-size: 30px; font-weight: bold; color: white;")

        self.video_container = QStackedWidget()
        self.video_container.addWidget(self.splash_label)
        self.video_container.addWidget(self.video_widget)
        self.video_container.setCurrentWidget(self.splash_label)

        self.btn_open = QPushButton(); self.btn_open_srt = QPushButton(); self.url_input = QLineEdit(); self.btn_load_url = QPushButton(); self.btn_toggle_url_bar = QPushButton(); self.btn_show_playlist = QPushButton(); self.btn_show_history = QPushButton()
        if qta: self.btn_open.setIcon(qta.icon('fa5s.folder-open')); self.btn_open_srt.setIcon(qta.icon('fa5s.closed-captioning')); self.btn_load_url.setIcon(qta.icon('fa5s.link')); self.btn_toggle_url_bar.setIcon(qta.icon('fa5s.globe')); self.btn_show_playlist.setIcon(qta.icon('fa5s.list')); self.btn_show_history.setIcon(qta.icon('fa5s.history'))
        self.url_input.setPlaceholderText("URL video (YouTube, dll)...")
        self.position_slider = ClickableSlider(Qt.Orientation.Horizontal); self.position_slider.setRange(0, 0); self.time_label = QLabel("00:00 / 00:00")
        self.btn_prev_playlist = QPushButton(); self.btn_next_playlist = QPushButton(); self.btn_play_pause = QPushButton(); self.btn_stop = QPushButton()
        if qta: self.btn_prev_playlist.setIcon(qta.icon('fa5s.step-backward')); self.btn_next_playlist.setIcon(qta.icon('fa5s.step-forward')); self.btn_play_pause.setIcon(qta.icon('fa5s.play')); self.btn_stop.setIcon(qta.icon('fa5s.stop'))
        self.btn_speed = QPushButton(f"{self.playback_speeds[self.current_speed_index]}x"); self.btn_mute = QPushButton()
        self.volume_slider = QSlider(Qt.Orientation.Horizontal); self.volume_slider.setRange(0, 100); self.volume_slider.setValue(self.last_volume); self.volume_slider.setFixedWidth(120)
        if qta: self._update_volume_icon()
        self.btn_mini_player = QPushButton(); self.btn_fullscreen = QPushButton(); self.btn_change_theme = QPushButton()
        if qta: self.btn_mini_player.setIcon(qta.icon('fa5s.window-minimize')); self.btn_fullscreen.setIcon(qta.icon('fa5s.expand')); self.btn_change_theme.setIcon(qta.icon('fa5s.palette'))

        self.controls_container = QWidget(); self.url_bar_widget = QWidget(); url_bar_layout = QHBoxLayout(); url_bar_layout.setContentsMargins(10, 0, 10, 5); url_bar_layout.addWidget(self.url_input); url_bar_layout.addWidget(self.btn_load_url); self.url_bar_widget.setLayout(url_bar_layout); self.url_bar_widget.setVisible(False)
        slider_layout = QHBoxLayout(); slider_layout.setContentsMargins(10, 0, 10, 0); slider_layout.addWidget(self.position_slider);
        bottom_controls_layout = QHBoxLayout(); bottom_controls_layout.setContentsMargins(10, 0, 10, 5); bottom_controls_layout.addWidget(self.btn_play_pause); bottom_controls_layout.addWidget(self.btn_stop); bottom_controls_layout.addWidget(self.btn_prev_playlist); bottom_controls_layout.addWidget(self.btn_next_playlist); bottom_controls_layout.addWidget(self.time_label); bottom_controls_layout.addStretch(1); bottom_controls_layout.addWidget(self.btn_open); bottom_controls_layout.addWidget(self.btn_toggle_url_bar); bottom_controls_layout.addWidget(self.btn_open_srt); bottom_controls_layout.addWidget(self.btn_speed); bottom_controls_layout.addWidget(self.btn_show_playlist); bottom_controls_layout.addWidget(self.btn_show_history); bottom_controls_layout.addWidget(self.btn_mute); bottom_controls_layout.addWidget(self.volume_slider); bottom_controls_layout.addWidget(self.btn_mini_player); bottom_controls_layout.addWidget(self.btn_fullscreen); bottom_controls_layout.addWidget(self.btn_change_theme)
        container_layout = QVBoxLayout(); container_layout.setContentsMargins(0, 5, 0, 0); container_layout.setSpacing(5); container_layout.addWidget(self.url_bar_widget); container_layout.addLayout(slider_layout); container_layout.addLayout(bottom_controls_layout); self.controls_container.setLayout(container_layout)
    ...
    def _connect_signals(self):
        self.btn_open.clicked.connect(self._open_file); self.btn_open_srt.clicked.connect(self._open_srt_dialog); self.url_input.returnPressed.connect(self._load_from_url); self.btn_load_url.clicked.connect(self._load_from_url); self.btn_toggle_url_bar.clicked.connect(self._toggle_url_bar); self.btn_play_pause.clicked.connect(self._toggle_play_pause); self.btn_stop.clicked.connect(self._stop_video); self.btn_fullscreen.clicked.connect(self._toggle_fullscreen); self.btn_mute.clicked.connect(self._toggle_mute); self.btn_speed.clicked.connect(self._change_playback_speed); self.btn_show_playlist.clicked.connect(self._toggle_playlist_window); self.btn_prev_playlist.clicked.connect(self._play_previous_video); self.btn_next_playlist.clicked.connect(self._play_next_video); self.btn_change_theme.clicked.connect(self._change_theme); self.btn_show_history.clicked.connect(self._show_history_window)
        self.history_window.history_item_selected.connect(self._play_from_history); self.history_window.delete_selected_requested.connect(self._delete_history_item); self.history_window.clear_all_requested.connect(self._clear_all_history_data)
        self.btn_mini_player.clicked.connect(self._show_mini_player); self.mini_player_widget.closing.connect(self._show_main_from_mini); self.mini_player_widget.btn_play_pause.clicked.connect(self._toggle_play_pause); self.mini_player_widget.btn_stop.clicked.connect(self._stop_video); self.mini_player_widget.position_slider.sliderMoved.connect(self._set_position)
        self.volume_slider.valueChanged.connect(self._set_volume); self.position_slider.sliderMoved.connect(self._set_position)
        ...
        self.audio_output.volumeChanged.connect(self._sync_main_volume_slider); self.playlist_widget.play_requested.connect(self._load_and_play_from_playlist); self.controls_hide_timer.timeout.connect(self._hide_controls)
        self.position_slider.hover_move.connect(self._show_thumbnail_preview); self.position_slider.hover_leave.connect(self.thumbnail_preview.hide); self.thumbnail_generator.thumbnail_ready.connect(self._update_thumbnail); self.request_thumbnail.connect(self.thumbnail_generator.generate, Qt.ConnectionType.QueuedConnection)

    @pyqtSlot("QVideoFrame")
    def process_frame_for_mini_player(self, frame):
        if not frame.isValid() or not self.mini_player_widget.isVisible():
            return
        qt_image = frame.toImage()
        self.mini_player_widget.update_frame(qt_image)
    
    # --- FIX: DEKORATOR @pyqtSlot(int) DIHAPUS UNTUK MENGATASI TypeError ---
    ...
    def _show_history_window(self): self.history_window.populate_list(); self.history_window.exec()
    def _add_to_history(self, path, title): self.history = [item for item in self.history if item.get('path') != path]; self.history.append({'path': path, 'title': title}); self.history = self.history[-50:]
    def _play_from_history(self, item): path = item.get('path'); self._load_video_file(path) if path else None
    def _delete_history_item(self, index): 
        if 0 <= index < len(self.history): del self.history[index]; self.history_window.populate_list(); self._save_config()
    def _clear_all_history_data(self): self.history.clear(); self.history_window.populate_list(); self._save_config()
    def dragEnterEvent(self, event): event.acceptProposedAction() if event.mimeData().hasUrls() else event.ignore()
    def dropEvent(self, event):
        for url in event.mimeData().urls():
            file_path = url.toLocalFile()
            if os.path.exists(file_path) and any(file_path.lower().endswith(ext) for ext in ['.mp4', '.mkv', '.webm', '.avi']):
                self._load_video_file(file_path)
                break
    def open_file_from_path(self, file_path): self._load_video_file(file_path)
    ...
    def _toggle_url_bar(self): self.url_bar_widget.setVisible(not self.url_bar_widget.isVisible())
    def _open_file(self):
    ...
    def _load_video_file(self, file_path_or_url):
        ...
        self._check_for_resume(path_for_history)
    
    def _save_current_position(self):
        if self.player.mediaStatus() == QMediaPlayer.MediaStatus.NoMedia or not self.current_media_info:
            return
        ...
        if path and duration > 0:
            if position > 5000 and (duration - position) > 5000:
                self.resume_points[path] = position
            elif path in self.resume_points:
                del self.resume_points[path]

    def _check_for_resume(self, path):
        saved_pos = self.resume_points.get(path)
        if saved_pos and saved_pos > 0:
            pos_time = QTime(0,0,0).addMSecs(saved_pos)
            reply = QMessageBox.question(self, "Lanjutkan Tontonan?",
                f"Video ini terakhir ditonton hingga {pos_time.toString('hh:mm:ss')}.\n\nLanjutkan dari sana?",
    ...
    def _load_and_play_from_playlist(self, file_path): self._load_video_file(file_path); self._update_playlist_nav_buttons()
    def _play_next_video(self):
        current_index = self.playlist_widget.get_current_index(); playlist_data = self.playlist_widget.get_playlist_data(); new_index = current_index + 1
        if 0 <= new_index < len(playlist_data): self._load_and_play_from_playlist(playlist_data[new_index]['path']); self.playlist_widget._update_selection(new_index)
    def _play_previous_video(self):
        current_index = self.playlist_widget.get_current_index(); playlist_data = self.playlist_widget.get_playlist_data(); new_index = current_index - 1
        if 0 <= new_index < len(playlist_data): self._load_and_play_from_playlist(playlist_data[new_index]['path']); self.playlist_widget._update_selection(new_index)
    def _update_playlist_nav_buttons(self):
        playlist_data = self.playlist_widget.get_playlist_data(); current_index = self.playlist_widget.get_current_index()
        self.btn_prev_playlist.setEnabled(current_index > 0); self.btn_next_playlist.setEnabled(current_index < len(playlist_data) - 1)
    def _toggle_playlist_window(self): self.playlist_widget.setVisible(not self.playlist_widget.isVisible())

    def _open_srt_dialog(self): file_path, _ = QFileDialog.getOpenFileName(self, "Pilih Subtitle", "", "SRT Files (*.srt)"); self._load_srt_file(file_path) if file_path else None
    def _time_to_ms(self, t): return (t.hour() * 3600 + t.minute() * 60 + t.second()) * 1000 + t.msec()
    def _load_srt_file(self, file_path):
        ...
        try:
            ...
            for block in subtitle_blocks:
                ...
                if len(lines) >= 3:
                    ...
                    if time_match:
                        start_time = QTime.fromString(time_match.group(1), "hh:mm:ss,zzz"); end_time = QTime.fromString(time_match.group(2), "hh:mm:ss,zzz")
        ...
        except Exception as e: QMessageBox.warning(self, "Error", f"Gagal memuat subtitle: {e}"); self.subtitles = []

    def resizeEvent(self, event):
        super().resizeEvent(event)
        video_rect = self.video_container.geometry()
        label_height = int(video_rect.height() * 0.20)
        label_y = video_rect.y() + video_rect.height() - label_height - 10
        self.subtitle_label.setGeometry(video_rect.x(), label_y, video_rect.width(), label_height)

    def _load_from_url(self):
        url = self.url_input.text().strip(); self.setWindowTitle("Macan Player - Mengambil info video..."); self.worker = YouTubeDLWorker(url); self.thread = threading.Thread(target=self.worker.run, daemon=True); self.worker.finished.connect(self._on_youtube_dl_finished); self.thread.start()
    def _on_youtube_dl_finished(self, video_url, title, error):
        if error or not video_url: QMessageBox.critical(self, "Error URL", error or "URL tidak valid."); self.setWindowTitle("Macan Player"); return
        self.current_media_info = {'path': video_url, 'title': title}; self._load_video_file(video_url)

    def _toggle_play_pause(self): self.player.play() if self.player.playbackState() != QMediaPlayer.PlaybackState.PlayingState else self.player.pause()
    def _stop_video(self):
        self._save_current_position()
        self.resume_save_timer.stop()
        self.player.stop(); self.subtitles = []; self._update_time_label(0, 0); self.position_slider.setValue(0); self._update_control_states()

    def _skip_forward(self): self._set_position(self.player.position() + self.SKIP_INTERVAL)
    def _skip_backward(self): self._set_position(max(0, self.player.position() - self.SKIP_INTERVAL))
    def _change_playback_speed(self): self.current_speed_index = (self.current_speed_index + 1) % len(self.playback_speeds); new_speed = self.playback_speeds[self.current_speed_index]; self.player.setPlaybackRate(new_speed); self.btn_speed.setText(f"{new_speed}x")
    def _update_position(self, position):
        if not self.position_slider.isSliderDown(): self.position_slider.setValue(position)
        self._update_time_label(position, self.player.duration()); self.mini_player_widget.update_position(position)
    def _set_position(self, position): self.player.setPosition(position)
    def _set_volume(self, value):
        self.audio_output.setVolume(value / 100.0)
        if value > 0 and self.audio_output.isMuted(): self.audio_output.setMuted(False)
        elif value == 0: self.audio_output.setMuted(True)
    def _toggle_mute(self): self.audio_output.setMuted(not self.audio_output.isMuted())
    def _update_volume_icon(self):
        if not qta: return
        volume = int(self.audio_output.volume() * 100); is_muted = self.audio_output.isMuted()
        if is_muted or volume == 0: icon = qta.icon('fa5s.volume-mute')
        elif 0 < volume <= 50: icon = qta.icon('fa5s.volume-down')
        else: icon = qta.icon('fa5s.volume-up')
        self.btn_mute.setIcon(icon)
    def _sync_main_volume_slider(self):
        volume_float = self.audio_output.volume(); is_muted = self.audio_output.isMuted()
        if not self.volume_slider.isSliderDown(): self.volume_slider.setValue(0 if is_muted else int(volume_float * 100))
        self._update_volume_icon()
    def _update_time_label(self, position, duration):
        if duration > 0:
            pos_time = QTime(0, 0, 0).addMSecs(position); dur_time = QTime(0, 0, 0).addMSecs(duration); fmt = 'hh:mm:ss' if duration >= 3600000 else 'mm:ss'
            self.time_label.setText(f"{pos_time.toString(fmt)} / {dur_time.toString(fmt)}")
        else: self.time_label.setText("00:00 / 00:00")

    def _update_duration(self, duration):
        self.position_slider.setRange(0, duration)
        self.mini_player_widget.update_duration(duration)
        if self.pending_resume_position != -1:
    ...
    def _update_control_states(self):
        ...
        self.video_container.setCurrentWidget(self.video_widget if is_media_loaded else self.splash_label)
        self.btn_play_pause.setEnabled(is_media_loaded); self.btn_stop.setEnabled(is_media_loaded); self.btn_mini_player.setEnabled(is_media_loaded)
        self._update_playlist_nav_buttons()

    def _update_play_pause_icon(self, state):
        is_playing = state == QMediaPlayer.PlaybackState.PlayingState
        if qta: self.btn_play_pause.setIcon(qta.icon('fa5s.pause') if is_playing else qta.icon('fa5s.play'))
        self.mini_player_widget.update_play_pause_icon(is_playing)

    def _handle_media_status_changed(self, status):
        if status == QMediaPlayer.MediaStatus.EndOfMedia:
            current_index = self.playlist_widget.get_current_index(); playlist_data = self.playlist_widget.get_playlist_data()
            self._play_next_video() if current_index < len(playlist_data) - 1 else self._stop_video()

    def _toggle_fullscreen(self):
        self.is_fullscreen = not self.is_fullscreen
        if self.is_fullscreen:
            self.normal_geometry = self.geometry(); self.showFullScreen()
        else:
            self.showNormal()
            if self.normal_geometry: self.setGeometry(self.normal_geometry)
            self.controls_container.setVisible(True); self.setCursor(Qt.CursorShape.ArrowCursor)

    def keyPressEvent(self, event):
        key = event.key()
        if key in (Qt.Key.Key_F, Qt.Key.Key_F11): self._toggle_fullscreen()
    ...
    def mouseMoveEvent(self, event):
        if self.is_fullscreen:
            self.controls_container.setVisible(True); self.setCursor(Qt.CursorShape.ArrowCursor); self.controls_hide_timer.start()
        super().mouseMoveEvent(event)
    ...
    def closeEvent(self, event):
        self._save_config()
        self.playlist_widget.close(); self.mini_player_widget.close(); self.history_window.close()
