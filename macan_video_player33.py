...
# --- PENAMBAHAN FITUR: MINI PLAYER WINDOW ---
class MiniPlayerWindow(QWidget):
    ...
    """
    # Sinyal untuk memberitahu jendela utama saat jendela ini ditutup
    closing = pyqtSignal()

    def __init__(self, player, audio_output, parent=None):
        super().__init__(parent)
        self.player = player # Berbagi instance player utama
        self.audio_output = audio_output # Berbagi instance audio output

        # --- PENAMBAHAN: Variabel status untuk kontrol volume ---
        ...
        # Inisialisasi status ikon berdasarkan status player saat ini
    ...
    def _setup_ui(self):
        ...
        self.setFixedSize(480, 320) # Ukuran tetap

        # --- KOREKSI: Tambahkan flag agar jendela selalu di atas (always on top) ---
        ...
        # --- PENAMBAHAN: Kontrol Volume & Mute ---
        ...
        main_layout.addWidget(self.video_widget, 1) # Video widget mengambil sisa ruang
    ...
    def _connect_signals(self):
        # Kontrol dari mini player ke media player
        self.btn_play_pause.clicked.connect(self._toggle_play_pause)
        self.btn_stop.clicked.connect(self._stop_video)
        self.position_slider.sliderMoved.connect(self._set_position)
        
        # --- PENAMBAHAN: Sinyal untuk kontrol volume ---
        self.btn_mute.clicked.connect(self._toggle_mute)
        self.volume_slider.valueChanged.connect(self._set_volume)
        
        # Update UI mini player dari status media player
        ...
        # --- PENAMBAHAN: Sinyal untuk sinkronisasi slider volume ---
    ...
    # --- PENAMBAHAN: Fungsi-fungsi untuk Volume ---
    ...
    def _toggle_mute(self):
        ...
        if self.is_muted:
            ...
            # Kembalikan ke volume terakhir atau default jika volume terakhir 0
    ...
    def _sync_volume_slider(self, volume_float):
        """Slot untuk menerima sinyal perubahan volume dari QAudioOutput."""
        value = int(volume_float * 100)
        # Hanya update jika slider tidak sedang digeser pengguna
        if not self.volume_slider.isSliderDown():
            self.volume_slider.setValue(value)

        # Update status mute dan ikon
        self.is_muted = value == 0
        self._update_volume_icon()
    # --- AKHIR PENAMBAHAN FUNGSI VOLUME ---
    ...
    def closeEvent(self, event):
        """Kirim sinyal saat jendela akan ditutup."""
        self.closing.emit()
...
# --- PENAMBAHAN BARU: JENDELA HISTORY (VERSI LENGKAP DENGAN TOMBOL HAPUS) ---
class HistoryWindow(QDialog):
    """Jendela untuk menampilkan riwayat video yang telah ditonton."""
    history_item_selected = pyqtSignal(dict)  # Mengirimkan dictionary item
    # --- PENAMBAHAN BARU: Sinyal untuk menghapus riwayat ---
    delete_selected_requested = pyqtSignal(int) # Mengirimkan indeks item yang akan dihapus
    clear_all_requested = pyqtSignal() # Sinyal untuk menghapus semua riwayat
    ...
    def _setup_ui(self):
        """Mengatur semua elemen UI di jendela ini."""
        ...
        self.main_layout.addLayout(button_layout) # Tambahkan layout tombol ke layout utama

    def _connect_signals(self):
        """Menghubungkan sinyal dari widget ke slot."""
        self.list_widget.itemDoubleClicked.connect(self._on_item_selected)
        # --- PENAMBAHAN BARU: Hubungkan tombol hapus ---
        self.btn_remove_selected.clicked.connect(self._remove_selected)
        self.btn_clear_all.clicked.connect(self._clear_all)

    def populate_list(self):
        """Mengisi list widget dengan data riwayat terbaru."""
        self.list_widget.clear()
        for item in reversed(self.history_data): # Tampilkan yang terbaru di atas
            list_item = QListWidgetItem(item.get('title', 'Judul Tidak Diketahui'))
            list_item.setData(Qt.ItemDataRole.UserRole, item) # Simpan seluruh data item
            self.list_widget.addItem(list_item)
            
    def _on_item_selected(self, item):
        """Memproses saat item di-double-click."""
        selected_data = item.data(Qt.ItemDataRole.UserRole)
        self.history_item_selected.emit(selected_data)
        self.accept() # Tutup jendela setelah dipilih
        
    # --- PENAMBAHAN BARU: Fungsi untuk tombol hapus ---
    def _remove_selected(self):
        """Memancarkan sinyal untuk menghapus item yang dipilih."""
        selected_items = self.list_widget.selectedItems()
    ...
    def _clear_all(self):
        """Meminta konfirmasi dan memancarkan sinyal untuk menghapus semua riwayat."""
...
class ModernVideoPlayer(QWidget):
    ...
    def __init__(self):
        ...
        # --- PENAMBAHAN BARU: Untuk Fitur Tema, Resume, dan History ---
        self.themes = {}
        self.theme_names = []
        self.current_theme_index = 0
        self.history = []
        self.current_media_info = {} # {'path': '...', 'title': '...'}
        self.auto_resume_timer = QTimer(self)
        self.auto_resume_timer.setInterval(5000) # Simpan posisi setiap 5 detik
        # --- AKHIR PENAMBAHAN BARU ---

        self.playlist_widget = PlaylistWidget()
        # --- PENAMBAHAN BARU: History Window ---
        ...
        # --- PENAMBAHAN: Inisialisasi Mini Player (sekarang dengan audio_output) ---
        self.mini_player_widget = MiniPlayerWindow(self.player, self.audio_output)

        # --- PENAMBAHAN BARU: Setup Tema ---
        self._setup_themes()
        ...
        # --- PENAMBAHAN BARU: Terapkan tema yang di-load ---
    ...
    def _apply_theme(self, theme_name):
        if theme_name in self.themes:
            ...
            # Setel stylesheet untuk window lain juga
    ...
    def _load_config(self):
        """Memuat konfigurasi volume, playlist, tema, resume, dan history."""
        try:
            ...
            # --- PENAMBAHAN BARU: Muat Tema, Resume, History ---
            # Muat tema
    ...
    def _save_config(self):
        ...
        # --- PENAMBAHAN BARU: Simpan posisi terakhir untuk auto-resume ---
        auto_resume_data = {}
        ...
        config = {
            ...
            # --- PENAMBAHAN BARU: Simpan Tema, Resume, History ---
    ...
    def _setup_ui(self):
        ...
        # Stylesheet awal akan ditimpa oleh _apply_theme
        ...
        # --- PENAMBAHAN BARU: Tombol History ---
        ...
        self.btn_mini_player.setEnabled(False) # Awalnya nonaktif
        ...
        bottom_controls_layout.addWidget(self.btn_show_history) # Tombol History
        ...
        bottom_controls_layout.addWidget(self.btn_change_theme) # Tombol Tema
    ...
    def _connect_signals(self):
        ...
        # --- PENAMBAHAN BARU: Sinyal untuk Tema dan History ---
        ...
        # --- BARIS BARU UNTUK MENGHUBUNGKAN SINYAL HAPUS DARI HISTORY WINDOW ---
    ...
    # --- PENAMBAHAN BARU: SLOT UNTUK FITUR BARU ---
    def _handle_auto_resume(self, resume_data):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if "://" in path: # Check if it's a URL
                self.player.setSource(QUrl(path))
            else:
                self.player.setSource(QUrl.fromLocalFile(path))
                
            # Beri sedikit waktu untuk media memuat sebelum set posisi
    ...
    def _save_current_position(self):
        if self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState:
            self._save_config() # Cukup panggil save config yang sudah diupdate
    ...
    def _add_to_history(self, path, title):
        # Hindari duplikat dengan menghapus entri lama jika ada
    ...
    # --- METODE BARU UNTUK MENGHAPUS RIWAYAT ---
    def _delete_history_item(self, index):
        """Menghapus satu item dari riwayat berdasarkan indeksnya."""
        # Karena daftar ditampilkan terbalik, kita perlu menghitung indeks yang benar.
        # Item di posisi 0 pada UI adalah item terakhir di dalam list self.history.
        actual_index = len(self.history) - 1 - index
        
        if 0 <= actual_index < len(self.history):
            del self.history[actual_index]
            self.history_window.populate_list() # Perbarui tampilan di jendela riwayat
            self._save_config() # Simpan perubahan ke file
            
    def _clear_all_history_data(self):
        """Menghapus semua data dari riwayat."""
        self.history.clear()
        self.history_window.populate_list() # Perbarui tampilan
        self._save_config() # Simpan perubahan
    # --- AKHIR PENAMBAHAN METODE BARU ---
    
    # --- AKHIR PENAMBAHAN BARU ---
    
    def _show_mini_player(self):
        """Sembunyikan jendela utama dan tampilkan pemutar mini."""
        self.player.setVideoOutput(self.mini_player_widget.video_widget)
        self.mini_player_widget.show()
        self.hide()

    def _show_main_from_mini(self):
        """Sembunyikan pemutar mini dan tampilkan kembali jendela utama."""
        self.player.setVideoOutput(self.video_widget)
        self.show()
    ...
    def _stop_video(self):
        ...
        self.auto_resume_timer.stop() # Hentikan timer saat video dihentikan
        self._update_playlist_nav_buttons()
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key.Key_F11: self._toggle_fullscreen()
            ...
            if self.is_fullscreen:
                ...
                if self.controls_container.isVisible(): self._show_cursor()
    ...
    def closeEvent(self, event):
        """Menyimpan konfigurasi sebelum aplikasi ditutup."""
        self._save_config()
