...
class ThumbnailGenerator(QObject):
    ...
    def generate(self, video_path, timestamp_ms, request_time):
        ...
        try:
            ...
            # Atur posisi video ke timestamp yang diinginkan (dalam milidetik)
...
class ModernVideoPlayer(QWidget):
    ...
    def _show_thumbnail_preview(self, x_pos):
        ...
        # --- PERBAIKAN: Cek apakah path adalah file lokal sebelum generate thumbnail ---
        is_url = "://" in video_path
        if not self.player.hasVideo() or self.player.duration() <= 0 or is_url or not os.path.exists(video_path):
            return
        ...
        # --- PERBAIKAN: Batasi permintaan thumbnail agar tidak overload ---
        if current_time - self.last_thumbnail_request_time > 0.1: # Max 10 thumbnail per detik
    ...
    def _toggle_fullscreen(self):
        ...
        if self.is_fullscreen:
            self.showFullScreen()
            # Kontrol disembunyikan lewat timer saat mouse tidak bergerak
        else:
            self.showNormal()
