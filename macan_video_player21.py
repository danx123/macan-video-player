...
class ModernVideoPlayer(QWidget):
    ...
    def __init__(self):
        ...
        # ==================== FIX DIMULAI DI SINI ====================
        # PINDAHKAN INISIALISASI TIMER KE SINI (SEBELUM _connect_signals DIPANGGIL)
        self.cursor_hide_timer = QTimer(self)
        self.cursor_hide_timer.setInterval(3000)
        self.cursor_hide_timer.setSingleShot(True) # Tambahkan ini agar lebih efisien
    ...
    def _setup_ui(self):
        ...
        # Center (Title)
        bottom_controls_layout.addWidget(self.title_label, 1) # Stretch factor of 1
    ...
    def _hide_controls(self):
        """Sembunyikan panel kontrol jika video sedang diputar dan tidak dalam mode fullscreen."""
        if self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState and not self.is_fullscreen:
    ...
    def _on_youtube_dl_finished(self, video_url, title, error):
        ...
        if video_url:
            ...
            if title: self.title_label.setText(f"{title}")
            self.player.play()
            self._update_playlist_nav_buttons()
    ...
    def _stop_video(self):
        self.player.stop()
        self.title_label.setText("Video dihentikan.")
        # self.playlist_widget.list_widget.clearSelection() # Optional: decide if you want to clear selection
        self._update_playlist_nav_buttons()
    ...
    def _handle_media_status_changed(self, status):
        ...
        if status == QMediaPlayer.MediaStatus.LoadedMedia and not self.is_fullscreen:
            ...
            if final_width < 640: final_width = 640
            if final_height < 480: final_height = 480
    ...
    def mouseMoveEvent(self, event):
        # Logika untuk fullscreen (menyembunyikan kursor)
        if self.is_fullscreen:
            self._show_cursor()
            self.cursor_hide_timer.start()

        # Logika untuk menampilkan/menyembunyikan kontrol di mode windowed
        else:
            # Tentukan area "panas" di bagian bawah jendela tempat kontrol berada
            controls_height = self.controls_container.height()
            hot_zone = QRect(0, self.height() - controls_height, self.width(), controls_height)

            if hot_zone.contains(event.pos()):
                # Jika kursor berada di area kontrol, tampilkan dan hentikan timer
                self.controls_container.setVisible(True)
                self.controls_hide_timer.stop()
            else:
