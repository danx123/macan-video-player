...
class YouTubeDLWorker(QObject):
    ...
    def run(self):
        ...
        try:
            # <<< FIX DI SINI >>> Menggunakan format yang lebih fleksibel dan andal
            ydl_opts = {
            ...
            with YoutubeDL(ydl_opts) as ydl:
                ...
                if video_url:
                    ...
                    # Mencoba format lain jika URL tidak ditemukan secara langsung
                    formats = info_dict.get('formats', [info_dict])
                    # Memilih format yang memiliki URL dan video+audio (jika ada)
                    for f in reversed(formats): # Prioritaskan kualitas lebih tinggi
                        if f.get('url') and f.get('vcodec') != 'none' and f.get('acodec') != 'none':
                             self.finished.emit(f['url'], None)
                             return
                    self.finished.emit(None, "Tidak dapat menemukan URL video streaming yang valid.")
