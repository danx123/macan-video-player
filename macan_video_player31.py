...
class ModernVideoPlayer(QWidget):
    ...
    def _setup_ui(self):
        ...
        # --- PENAMBAHAN: Tambahkan tombol mini player ke layout ---
    ...
    def _connect_signals(self):
        ...
        # --- PENAMBAHAN: Hubungkan sinyal untuk mini player ---
        ...
        # --- PENAMBAHAN: Sinyal untuk sinkronisasi slider volume utama ---
    ...
    # --- PENAMBAHAN: Slot untuk menampilkan/menyembunyikan mini player ---
    def _show_mini_player(self):
        """Sembunyikan jendela utama dan tampilkan pemutar mini."""
        # Alihkan output video ke widget di pemutar mini
    ...
    def _show_main_from_mini(self):
        ...
        # Kembalikan output video ke widget utama
        self.player.setVideoOutput(self.video_widget)
        self.show()
    ...
    def _time_to_ms(self, t):
        """Mengonversi objek QTime ke milidetik."""
        # FIX: Panggil metode dengan () -> t.hour(), t.minute(), t.second(), t.msec()
        return (t.hour() * 3600 + t.minute() * 60 + t.second()) * 1000 + t.msec()

    def _load_srt_file(self, file_path):
        """
        Memuat dan mem-parsing file subtitle .srt dengan cara yang lebih tangguh.
        """
        self.subtitles = []
        try:
            ...
            # Mengganti baris baru Windows (\r\n) menjadi standar (\n)
            # dan memisahkan file menjadi blok-blok subtitle
            subtitle_blocks = content.strip().replace('\r\n', '\n').split('\n\n')

            for block in subtitle_blocks:
                lines = block.split('\n')
                # Blok yang valid setidaknya memiliki 3 baris: nomor, waktu, dan teks
                if len(lines) >= 3:
                    # Baris kedua harus berisi timestamp
                    time_line = lines[1]
                    # Baris selanjutnya adalah teks subtitle itu sendiri
                    text = "\n".join(lines[2:])

                    # Regex untuk mengekstrak waktu mulai dan selesai
                    ...
                    if time_match:
                        ...
                        # Tambahkan data subtitle ke dalam list
    ...
    def _update_subtitle(self, position):
        """
        Menampilkan subtitle yang sesuai berdasarkan posisi video saat ini.
        """
        if not self.subtitles:
            return

        current_text = ""
        # Cari subtitle yang rentang waktunya cocok dengan posisi video
        for sub in self.subtitles:
            if sub['start_ms'] <= position <= sub['end_ms']:
                current_text = sub['text']
                break  # Hentikan pencarian setelah menemukan yang cocok

        # Tampilkan atau sembunyikan label subtitle
        if current_text:
    ...
    def _sync_main_volume_slider(self, volume_float):
        """Slot untuk menyinkronkan slider volume utama."""
        value = int(volume_float * 100)
        if not self.volume_slider.isSliderDown():
            self.volume_slider.setValue(value)
    ...
    def _update_time_label(self, position, duration):
        ...
        # Tentukan format string berdasarkan total durasi video
    ...
    def _update_control_states(self):
        ...
        self.btn_mini_player.setEnabled(is_media_loaded) # Aktifkan tombol mini player jika ada media
        self._update_play_pause_icon(is_playing)
        self._update_playlist_nav_buttons()
    ...
    # --- PERUBAHAN FUNGSI DIMULAI: Logika mouse di fullscreen diubah ---
    def mouseMoveEvent(self, event):
        if self.is_fullscreen:
            # Di mode fullscreen, pergerakan mouse sekarang hanya menampilkan kursor.
            # Bilah navigasi dikontrol secara manual dengan tombol Spasi.
            self._show_cursor()
            # Mulai ulang timer untuk menyembunyikan kursor
    ...
    # --- PERUBAHAN FUNGSI SELESAI ---

    # --- PERUBAHAN FUNGSI DIMULAI: Modifikasi tombol Spasi ---
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key.Key_F11: self._toggle_fullscreen()
            ...
            if self.is_fullscreen:
                # Di mode fullscreen, Spasi berfungsi untuk menampilkan/menyembunyikan bilah navigasi.
                # Fungsi play/pause dapat diakses dengan mengklik video.
                is_visible = self.controls_container.isVisible()
                self.controls_container.setVisible(not is_visible)
                
                # Saat kontrol ditampilkan, pastikan kursor juga terlihat.
    ...
    def eventFilter(self, source, event):
        if source is self.video_widget:
            # --- FIX BARU DIMULAI: Menangani Drag & Drop pada Video Widget ---
            if event.type() == QEvent.Type.DragEnter:
                # Meneruskan event ke handler utama agar kursor berubah
                self.dragEnterEvent(event)
                return True # Event sudah ditangani
            elif event.type() == QEvent.Type.Drop:
                # Meneruskan event ke handler utama untuk memutar video
    ...
    def closeEvent(self, event):
        ...
        self.mini_player_widget.close() # Pastikan mini player juga ditutup
        event.accept()
